-- ============================================
-- SCRIPT CORRECTO: LOCALES Y ASIGNACIONES
-- ============================================
-- 1. Elimina los locales incorrectos creados anteriormente
-- 2. Crea los locales correctos según locales.md
-- 3. Asigna correctamente cada local a su usuario
-- ============================================

-- PASO 1: Eliminar locales incorrectos que se crearon antes
-- ============================================
DELETE FROM locales WHERE local IN (
  'UFB09', 'UFC04', 'UFC08', 'UFP14', 'UFP19',
...
-- Locales para Adm Constitucion (42 locales)
...
-- Locales para Adm Zona Sur (49 locales)
INSERT INTO locales (local, categoria) VALUES
('UFB110', 'Trenes'),
('UFB31', 'Trenes'),
('UFB32', 'Trenes'),
...
-- Asignaciones para Adm Constitucion (42 locales)
...
-- Asignaciones para Adm Oeste (21 locales - UFB12 ya está en Constitucion)
INSERT INTO usuario_locales (usuario_id, local)
SELECT u.id, l.local
FROM usuarios u
CROSS JOIN (
...
-- Asignaciones para Adm Zona Sur (49 locales)
INSERT INTO usuario_locales (usuario_id, local)
SELECT u.id, l.local
FROM usuarios u
CROSS JOIN (
...
-- Ver detalle de locales por usuario
SELECT
  u.nombre as usuario,
  STRING_AGG(ul.local, ', ' ORDER BY ul.local) as locales
FROM usuarios u
LEFT JOIN usuario_locales ul ON u.id = ul.usuario_id
WHERE u.nombre LIKE 'Adm%'
GROUP BY u.nombre
